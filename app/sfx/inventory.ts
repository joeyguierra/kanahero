...
export const CUES: Cue[] = [
  ...
  },
  {
    ...
    note: "The fanfare, with no melody: one deck squared. It was the closing half of a bookend — deal.press opened the run — and since 2026-09-25 it stands alone: DEAL is silent, so this is the only time the deck is heard as a deck.",
    rig: "reveal",
  },
  {
    name: "reveal.skip",
  ...
  // ---- tier 2 · deals and arrivals ---------------------------------------
  {
    ...
    capMs: 400,
    peakDb: -12,
    where: "nowhere — unwired 2026-09-25. Was: DEAL, the S6b CTA",
    note: "A deck squared and cut once: the level-start, and the bookend to reveal.end. Still the best single non-loop sound outside S8, and still baked — it lost its call site, not its argument. Audition it here; it is heard nowhere in the app.",
    amended: "DROPPED FROM DEAL, 2026-09-25. onDeal() mounts S7 in the same handler, so 400 ms of riffle played over the new screen and read as a stray reveal.skip. DEAL is silent now — and must not inherit ui.primary instead, which is longer.",
    rig: "deal",
  },
  {
    name: "prompt.melt",
  ...
  // ---- tier 3 · chrome ----------------------------------------------------
  {
    ...
    tier: 3,
    material: "cardstock",
    spec: "≤150 ms · −22 dBFS · 300–800 Hz",
    capMs: 150,
    peakDb: -22,
    gapMs: 150,
    where: "← back · VIEW COLLECTION → · CREDITS · BACK TO DECK · ✕ on a capture · every phase change in app/page.tsx",
    note: "One dry cardstock tap. The cursor sound — and the screen transition too: there is no separate transition cue. It lives on go() in the state machine rather than on the buttons, so a screen added later inherits it by being a screen. Deck and set rows are NOT here: choosing is drawer.open.",
    amended: "150 ms / −22 by ear, 2026-09-25, against the inventory's 50 ms / −20. Kept: the tap has a body that a 50 ms cap cut off, and the extra level came back off in exchange. Note the cap now equals the gap, so two fast back-taps meet end to end.",
    rig: "chrome",
  },
  {
    name: "drawer.open",
  ...
  },
  {
    ...
    capMs: 725,
    peakDb: -14,
    where: "START SESSION · OPEN BANK · CHARACTERS · Replay missed · Again — the bone and strike CTAs without a cue of their own",
    note: "A brass latch closing, heavier than nav. The confirm — and it comes AFTER the choice it confirms has already sounded, which is why S1 gives you drawer.open then this. FLIP is excluded: it has ink.reveal, because what it confirms is a specific object. DEAL is excluded too, but for a different reason — it is deliberately silent (2026-09-25), because the round mounts in the same handler and anything fired there is heard on the next screen. KEEP WRITING is modal.close, not this: the dialog is the object.",
    amended: "725 ms by ear, 2026-09-25, against the inventory's 120. Long for a confirm, and accepted: every site it fires from leads to a screen that takes longer than that to arrive, so unlike DEAL it is not heard over anything.",
    rig: "chrome",
  },
  {
    name: "ui.toggle (ON)",
...
export const LADDER: { db: string; width: number; cues: string; hot?: boolean }[] = [
  ...
  { db: "−12", width: 82, cues: "hand.tick · reveal.end · reveal.skip · deal.press (unwired)" },
